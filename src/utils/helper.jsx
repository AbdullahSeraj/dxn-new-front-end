...
export const Stars = ({ number }) => {
  ...
  return (
    <div className="flex justify-center items-center gap-[3px] my-2">
      {[...Array(starNumber)].map((star, i) => {
        if (num > 0.5) {
          num--;
          return <FaStar key={i} className="text-amber-300 text-xs" />
        } else if (num <= 0.5 && num > 0) {
          num = num - 0.5;
          return <FaStarHalfStroke key={i} className="text-amber-300 text-xs" />
        } else
          return <FaRegStar key={i} className="text-amber-300 text-xs" />
      })}
    </div>
  )
}
...
export const resetStringLength = (name, length) => {
  let newName = "";
  if (name) {
    if (name.length > length) {
      newName = name.slice(0, length) + "..."
    } else {
      newName = name
    }
  }
...
export const filterArray = (array, maxLength, shuffle, group = "") => {
  // const { data: settings } = useSettingsQuery()

  let newArray = array;

  if (group != "") {
    newArray = newArray.filter((arr) => arr.group == group)
  }

  if (shuffle) {
    newArray = shuffleFun(newArray);
  }
...
export const isNew = (createAt) => {
  const createdDate = new Date(createAt);
  const nowDate = new Date();
  const date = nowDate - createdDate;

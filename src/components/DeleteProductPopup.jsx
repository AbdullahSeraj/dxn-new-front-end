...
import { useDeleteProductMutation } from '../redux/features/productApiSlice'
import { handleClickTrue } from '../redux/slices/SnackbarSlice'
import { useDispatch } from 'react-redux'

const DeleteProductPopup = ({ id, handleShowDeleteProduct }) => {
    const dispatch = useDispatch()
    const [deleteUser, { isLoading, isError, error }] = useDeleteProductMutation()
        ...
        <div className='fixed top-0 left-0 bottom-0 right-0 z-[300]'>
            ...
            <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] max-md:w-[calc(100vw-50px)] bg-white rounded-lg px-10 py-5'>
                <>
                    ...
                    <div className='my-5'>
                        <h3 className='text-center font-semibold text-sm'>Are you sure to want to delete this product</h3>
                        ...
                        <div className='flex items-center gap-5'>
                            ...
                            <div className='flex items-center justify-center flex-1'>
                                <button onClick={handleUpdateRoleUser} className='bg-red-600 hover:bg-red-600/90 text-white rounded-md py-2 w-full text-xs font-semibold mt-5 shadow-md disabled:bg-red-200 disabled:cursor-not-allowed' disabled={isLoading} >{isLoading ? "Loading..." : "Delete"}</button>
                            </div>
                        </div>
                    </div>
                </>

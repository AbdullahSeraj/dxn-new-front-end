...
import { useProductQuery, useUpdateProductMutation } from "../redux/features/productApiSlice"
...
const EditProductPopup = ({ id, handleShowEditProduct }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await updateProduct({ id: id, data: productInputs })
    ...
    return (
        <div className='fixed top-0 left-0 bottom-0 right-0 z-[300]'>
            <div className='fixed top-0 left-0 bottom-0 right-0 bg-gray-500/70' onClick={handleShowEditProduct}></div>
                    ...
                    <>
                        <div className='sticky top-0 right-0 flex justify-end z-50'>
                            <div className='bg-white absolute top-0 left-0 w-full min-h-[50px] flex justify-between items-center py-5 px-5'>
                                <div className=''>
                                    <h2 className='text-text font-bold text-xl'>Update Product</h2>
                                </div>

                                <button className='bg-red-600/20 hover:bg-red-600/30 rounded-full p-1 w-fit z-[400]' onClick={handleShowEditProduct}>
                        ...
                        <div className="px-7 pb-7 pt-[70px]">
                            <form onSubmit={handleSubmit}>
                                ...
                                <fieldset className='flex flex-col my-3 w-full'>
                                    ...
                                    <textarea rows={5} type="text" name="description" id="description" placeholder='description' className='input resize-none' value={productInputs.description} onChange={(e) => setProductInputs((prev) => { return { ...prev, description: e.target.value } })} />
                                    {/* {errors.usernameError ? <div className='text-red-600 mt-1 text-xs'>{errors.usernameError}</div> : null} */}
                                </fieldset>
                                ...
                                <fieldset className='flex flex-col my-3 w-full'>
                                    ...
                                    <select name="group" id="group" className='input' value={productInputs?.group} onChange={(e) => setProductInputs((prev) => { return { ...prev, group: e.target.value } })}>
                                ...
                                <fieldset className='flex flex-col my-3 w-full'>
                                    ...
                                    <select name="category" id="category" className='input' value={productInputs?.category} onChange={(e) => setProductInputs((prev) => { return { ...prev, category: e.target.value } })}>
                                        <option value="">Select...</option>
                                ...
                                <div className='overflow-auto w-full'>
                                    <div className='w-full max-w-full min-w-fit'>
                                        <div className="flex gap-5 mt-3 overflow-x-auto w-full max-w-full min-w-full">
                                ...
                                <fieldset>
                                    <button className="bg-blue-600 text-white w-full py-2 px-5 font-semibold rounded-md shadow-md mt-4 hover:bg-blue-500 disabled:bg-blue-200 disabled:cursor-not-allowed" disabled={isLoadingUpdateProduct}>{isLoadingUpdateProduct ? "Loading..." : "Update Product"}</button>
                                </fieldset>
                            </form>
                        </div>
                    </>
